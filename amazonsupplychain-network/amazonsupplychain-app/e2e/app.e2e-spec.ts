...
describe('Starting tests for amazonsupplychain-app', function() {
  ...
  it('website title should be amazonsupplychain-app', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('amazonsupplychain-app');
    })
  });

  it('network-name should be amazonsupplychain-network@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('amazonsupplychain-network@0.0.1.bna');
    });
  });

  it('navbar-brand should be amazonsupplychain-app',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('amazonsupplychain-app');
    });
  });
